...
const NexusLogo = ({ isLoading }) => (
  ...
  >
    ...
    {isLoading && (
      <path
        d="M0 10H40M0 20H40M0 30H40M10 0V40M20 0V40M30 0V40"
        stroke="#1e293b"
        strokeWidth="0.5"
      />
    )}
...
function App() {
  ...
  const searchJobs = async (targetPage = 1) => {
    try {
      ...
      const url = `https://jobfinderfastapi.onrender.com/jobs?title=${encodeURIComponent(
        title
      )}&location=${encodeURIComponent(location)}&page=${targetPage}&limit=${limit}`;
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        {showNotice && (
          <div className="fixed inset-0 z-100 flex items-center justify-center bg-slate-900/60 px-4">
            <div className="w-full max-w-2xl border-2 border-slate-900 bg-white p-5 sm:p-7 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)]">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  <NexusLogo isLoading={false} />
                  <div>
                    <p className="font-mono text-[10px] sm:text-xs font-black uppercase tracking-[0.25em] text-slate-500">
                      Deployment Notice
                    </p>
                    <h2 className="mt-1 text-xl sm:text-2xl font-black uppercase tracking-tight">
                      Project Information
                    </h2>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="border border-slate-300 p-2 hover:border-slate-900 transition"
                >
                  <X size={16} />
                </button>
              </div>

              <div className="mt-6 space-y-4 text-sm sm:text-base leading-6 text-slate-700">
                <p>
                  This project is developed to scrape jobs from platforms like
                  Indeed, Naukri, Internshala, and similar sources.
                </p>
                <p>
                  The full multi-source version works locally, but some
                  platforms restrict cloud deployment because of anti-bot
                  protections.
                </p>
                <p>
                  The source code is available on{" "}
                  <a
                    href="https://github.com/SoumyaranjanDS/"
                    target="_blank"
                    rel="noreferrer"
              ...
              <div className="mt-6 flex justify-end">
                <button
                  onClick={() => setShowNotice(false)}
                  className="border-2 border-slate-900 bg-slate-900 px-5 py-2 text-xs sm:text-sm font-mono font-black uppercase text-white hover:bg-white hover:text-slate-900 transition"
                >
                  Continue
                </button>
              </div>
        ...
        <nav className="border-b border-slate-900 bg-white sticky top-0 z-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 sm:h-16 flex items-center justify-between">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 text-[10px] sm:text-xs font-mono font-bold border-2 transition-all ${
                showSaved
                  ? "bg-blue-600 border-blue-600 text-white"
                  : "border-slate-900 hover:bg-slate-100"
              }`}
            >
              <Heart
                size={12}
              ...
              />
              <span className="hidden xs:inline">SAVED_DATA</span> (
              {savedJobs.length})
            </button>
          </div>
        </nav>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 w-full flex-1">
          <section className="max-w-4xl mx-auto mb-10 sm:mb-16">
            ...
            {!showSaved && (
              <div className="border-[3px] sm:border-4 border-slate-900 bg-white p-1.5 sm:p-2 flex flex-col md:flex-row gap-2 shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] sm:shadow-[8px_8px_0px_0px_rgba(15,23,42,1)]">
                <div className="flex-1 flex items-center px-3 sm:px-4 py-3 gap-3 border-b-2 md:border-b-0 md:border-r-2 border-slate-100">
                  ...
                  <input
                    className="bg-transparent w-full text-xs sm:text-sm font-mono focus:outline-none font-bold uppercase"
                ...
                <button
                  ...
                  className="bg-blue-600 text-white px-6 sm:px-10 py-3 sm:py-4 font-mono font-black text-xs sm:text-sm hover:bg-slate-900 transition-colors uppercase flex items-center justify-center gap-2 active:translate-x-1 active:translate-y-1"
                >
            ...
            {error && (
              <p className="mt-6 text-[10px] sm:text-xs font-mono font-bold text-red-600 bg-red-50 p-3 border border-red-200">
                {error}
              </p>
            )}
          </section>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 pb-2 border-b-2 border-slate-900 gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                className="bg-transparent text-[10px] sm:text-xs font-mono font-bold focus:outline-none uppercase cursor-pointer"
                value={sourceFilter}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {displayJobs.map((job, index) => {
              const isSaved = savedJobs.some(
                (s) => s.apply_link === job.apply_link
              );

              return (
                <div
                  key={job.apply_link || index}
                  className="group relative flex flex-col p-5 sm:p-6 border-2 border-slate-900 bg-white hover:bg-slate-50 transition-all duration-200 hover:shadow-[4px_4px_0px_0px_rgba(15,23,42,1)]"
                >
                  <div className="flex justify-between items-start mb-4 sm:mb-6 gap-3">
                    <span className="text-[9px] sm:text-[10px] font-mono font-black px-2 py-0.5 sm:py-1 bg-slate-900 text-white uppercase shrink-0">
                      {job.source}
                    </span>
                    ...
                    <button
                      ...
                      className={`p-2 border border-slate-200 transition-all ${
                        isSaved
                          ? "bg-blue-600 border-blue-600 text-white"
                          : "hover:border-slate-900 bg-white"
                      }`}
                    >
                      <Heart
                        size={16}
                  ...
                  <h3 className="text-base sm:text-lg font-black leading-tight mb-1 uppercase tracking-tighter line-clamp-2">
                    {job.title}
                  </h3>

                  <p className="text-[11px] sm:text-xs font-mono font-bold text-blue-600 mb-6 sm:mb-8 truncate">
                    {job.company}
                  </p>

                  <div className="space-y-3 mb-8 sm:mb-10 font-mono">
                    <div className="flex items-center gap-3 text-[10px] sm:text-[11px] font-bold uppercase">
                    ...
                    <div className="flex items-center gap-3 text-[10px] sm:text-[11px] font-bold opacity-50 uppercase">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-auto flex items-center justify-center gap-2 w-full py-3 sm:py-4 bg-white border-2 border-slate-900 text-slate-900 font-mono font-black text-[10px] sm:text-xs uppercase hover:bg-slate-900 hover:text-white transition-all active:translate-x-0.5 active:translate-y-0.5"
                  >
          ...
          {!showSaved && totalResults > 0 && (
            <div className="mt-12 sm:mt-16 pt-8 border-t-2 border-slate-900 flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6 font-mono">
              <button
                ...
                className="w-full md:w-auto px-8 py-3 border-2 border-slate-900 font-black text-xs uppercase hover:bg-slate-100 disabled:opacity-20 transition-all"
              >
                {"< PREV_PAGE"}
              </button>

              <span className="font-black text-xs sm:text-sm bg-slate-100 px-4 py-2 border border-slate-900">
          ...
          {displayJobs.length === 0 && !loading && (
            <div className="text-center py-24 sm:py-32 border-2 border-dashed border-slate-300">
              <p className="font-mono font-black text-slate-300 uppercase tracking-[0.2em] sm:tracking-[0.5em] text-sm sm:text-base px-4">
                System.Empty_Result_Set
              </p>
            </div>
          )}
        </main>
        ...
        <footer className="border-t-2 border-slate-900 py-8 bg-white">
          <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6 font-mono uppercase">
            ...
            <div className="flex gap-8 text-slate-900">
              ...
              <a
                href="https://www.linkedin.com/in/soumyaranjanlink/"
                className="hover:text-blue-600 transition-colors"
                target="_blank"
                rel="noreferrer"
              >
              ...
              <a
                href="https://www.instagram.com/_.soumya_28?igsh=MW51OTV2bnc3aHdxaQ=="
                className="hover:text-blue-600 transition-colors"
                target="_blank"
                rel="noreferrer"
              >
